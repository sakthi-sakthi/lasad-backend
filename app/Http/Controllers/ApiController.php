...
class ApiController extends Controller
{
    ...
    public function storecontact(Request $request)
    {
        ...
        $bodyContent = [
            ...
            'tomobile'=> $validatedData['mobile'],
            'tosubject'=> $validatedData['message'],
        ];

        try {
            Mail::to('sakthiganapathis97@gmail.com')->send(new ContactFormMail($bodyContent));
    ...
    public function storevocation(Request $request)
    {
        try {
            $request->validate([
                'name' => 'required',
                ...
                'numberOfSisters' => 'nullable',
                'numberOfBrothers' => 'nullable',
            ]);
            Vocation::create($request->all());
            $bodyContent = [
                'name' => $request['name'],
                'email' => $request['email'],
                ...
                'numberOfSisters' => $request['numberOfSisters'],
                'numberOfBrothers' => $request['numberOfBrothers'],
            ];
            Mail::to('sakthiganapathis97@gmail.com')->send(new VocationFormMail($bodyContent));
            return response()->json(['status' => 'success', 'message'=> 'Request sent successfully']);
        } catch (\Throwable $th) {
...
,                'category_name' => $slides->category_name,
    ...
    public function getinstitution($id){
        $institutions = Institution::select(
            ...
            'institutions.year',
            'institutions.place',
            'institutions.media_id',
            'institutions.category_id',
            'institutions.status',
            'categories.title as category_name',
        )
            ->leftJoin('categories', 'institutions.category_id', '=', 'categories.id')
            ->where('institutions.status', 1)
            ->get();
        
        $institutions->each(function ($institution) {
            $mediaUrl = null;
            $media = Media::find($institution->media_id);
        
            if ($media) {
                $mediaUrl = $media->getUrl();
                $mediathumb = $media->getUrl('thumb');
            }
            if($institution->media_id != 1){
                $institution->image = $mediaUrl;
                $institution->mediathumb = $mediathumb;
            }
        });
        return response()->json([
            'success' => true,
