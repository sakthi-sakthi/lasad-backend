    ...
    <div class="content-wrapper">
        ...
        <div class="content-header"> 
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h4 class="m-0 text-dark">{{ __('Edit Institutions') }}</h4>
                    </div><!-- /.col -->
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            ...
                            <li class="breadcrumb-item"><a
                                    href="{{ route('admin.institution.index') }}">{{ __('Institutions') }}</a></li>
                                 
                                 <li class="breadcrumb-item active">{{ $institutions->title }}</li>
                           </ol>
                        </ol>
                    </div><!-- /.col -->
                </div><!-- /.row -->
        ...
        <div class="content">
            <div class="container-fluid">
                <form action="{{ route('admin.institution.update',$institutions->id) }}" method="post" id="form" enctype="multipart/form-data">
                    @method('PUT')
                    @csrf
                    <div class="row">
                            ...
                            <div class="card">
                                ...
                                <div class="card-body">
                                    <input type="hidden" id="category_id" name="category_id"
                                        value="{{ old('category_id') ?? $institutions->getCategory->id }}">
                                    <input type="hidden" name="media_id" id="media_id"
                                        value="{{ old('media_id') ?? $institutions->getMedia->id }}">
                                    <div class="form-group">
                                        <label for="title">{{ __('Name of the Institutions') }} <span class="text-danger">*</span> </label>
                                        <input type="text"
                                            class="form-control form-control-sm @error('title') is-invalid @enderror"
                                            id="title" name="title" value="{{ old('title') ?? $institutions->title }}">
                                        @error('title') <small
                                            class="ml-auto text-danger">{{ __('main.titleError') }}</small> @enderror
                                    </div>
                                    <div class="form-group">
                                        <label for="place">{{ __('Place') }}</label>
                                        <input type="text" class="form-control form-control-sm" id="place" name="place" value="{{ old('place') ?? $institutions->place }}">
                                     @error('place')
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea class="form-control form-control-sm" rows="3" id="content"
                                            name="content">{{ old('content') ?? $institutions->content }}</textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="card">
                                ...
                                <div class="card-body">
                                    <img src="{{ $institutions->getMedia->id == 1 ? '' : $institutions->getMedia->getUrl() ?? '' }}"
                            ...
                            <div class="card">
                                <div class="card-header">
                                    <label>{{ __('Year of Establishment') }}</label>
                                </div>
                                <div class="card-body">
                                    <div class="form-group">
                                      <input type="text" class="form-control form-control-sm" id="year" name="year" value="{{ old('year') ?? $institutions->year }}">
                                    </div>
                                </div>
                            </div>
                            <div class="card">
                                    ...
                                    <div class="form-group">
                                        <select class="form-control form-control-sm" id="category" name="category_id">
                                            ...
                                            @foreach ($categories as $category)
                                                <option value="{{ $category->id }}" @if (old('category') ?? $institutions->getCategory->id == $category->id) selected @endif>
                        ...
                        <div class="card" id="save-card">
                            <div class="card-body">
                                <a href="javascript:void(0);" class="btn btn-success btn-sm float-right"
                                    ...
                                    <a href="{{ route('admin.institution.index') }}" class="btn btn-danger btn-sm float-right mr-2">Cancel</a>
                            </div>
                        </div>
                    </div>
                </form>
